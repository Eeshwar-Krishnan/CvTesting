...
public class TowerGoalTester {

	public static void main(String[] args) throws IOException {
		System.loadLibrary(Core.NATIVE_LIBRARY_NAME);
		BufferedImage in = ImageIO.read(new File("D:\\Documents\\Code\\Python\\Tensorflow\\ObjDec\\workspace\\training_demo\\images\\test\\output1.png"));
 ...
 * double d = Imgproc.arcLength(mat2f, true);
			...
			//Imgproc.putText(testMat, ""+approxCurve.toArray().length, mat2f.toArray()[mat2f.toArray().length/2], 1, 1.5, new Scalar(255, 255, 255));
			List<MatOfPoint> dpArr = new ArrayList<>();
			MatOfPoint approx = new MatOfPoint();
			approxCurve.convertTo(approx, CvType.CV_32S);
			dpArr.add(approx);
			List<KeyPoint> kp = new ArrayList<>();
			for(Point p : approx.toArray()) {
				//Imgproc.circle(testMat, p, 1, new Scalar(0, 0, 255));
				kp.add(new KeyPoint((float)p.x, (float)p.y, 10));
			}
			
			d = Imgproc.arcLength(new MatOfPoint2f(match.toArray()), true);
			approxCurve = new MatOfPoint2f();
			Imgproc.approxPolyDP(new MatOfPoint2f(match.toArray()), approxCurve, 0.005 * d, false);
			dpArr = new ArrayList<>();
			approx = new MatOfPoint();
			approxCurve.convertTo(approx, CvType.CV_32S);
			dpArr.add(approx);
			Imgproc.cvtColor(inMatM, inMatM, Imgproc.COLOR_GRAY2RGB);
			List<KeyPoint> qKp = new ArrayList<>();
			for(Point p : approx.toArray()) {
				//Imgproc.circle(inMatM, p, 1, new Scalar(0, 0, 255));
				qKp.add(new KeyPoint((float)p.x, (float)p.y, 0));
			}
			
			BRISK brisk = BRISK.create();
			Mat desc = new Mat();
			brisk.compute(testMat, new MatOfKeyPoint(kp.toArray(new KeyPoint[1])), desc);
			
			Mat qDesc = new Mat();
			brisk.compute(inMatM, new MatOfKeyPoint(qKp.toArray(new KeyPoint[1])), qDesc);
			
			DescriptorMatcher matcher = DescriptorMatcher.create(DescriptorMatcher.BRUTEFORCE_HAMMING);
			MatOfDMatch matches = new MatOfDMatch();
			matcher.match(qDesc, desc, matches);
*/
